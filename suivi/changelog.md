...
- **Admin 0.38.0 — Sécurité › Synthèse unique et fenêtre « Moteurs de sécurité »** : la Synthèse est un seul composant (`security-synthese.js`) pour l'Admin et les passerelles, qui remplace les deux anciennes pages. Elle affiche un score de sécurité en anneau, quatre indicateurs (bans actifs, menaces actives, CVE critiques, certificats), les pastilles d'état des moteurs, l'activité sur 24 h, les bans actifs par source, les menaces récentes, un classement d'exposition (passerelles côté Admin, backends côté passerelle), la posture (en-têtes, certificats) et la timeline de sécurité filtrable par type. L'icône réglages ouvre la fenêtre **Moteurs de sécurité** : un onglet par moteur avec un interrupteur général et un interrupteur par capacité, appliqué immédiatement. Sentinel (propre à la passerelle côté passerelle) : bannir les IP détectées (mode `block`/`detect`), listes d'IP, User-Agent, chemins, débit par IP, erreurs 4xx, limite globale, tarpit. Fail2Ban : activation, confiance en `X-Forwarded-For`, ban permanent, réglages avancés. CrowdSec : activation et réglages avancés. Règles automatiques (Admin) : activation de chaque règle. Scanner CVE : réseaux privés. Côté passerelle, Fail2Ban et CrowdSec étant globaux, l'onglet « Moteur IPS » choisit le fournisseur de la passerelle (natif, Fail2Ban, CrowdSec). Sur mobile (≤ 720 px), les indicateurs passent sur deux colonnes, la timeline et le classement s'empilent, et la fenêtre devient plein écran avec des onglets défilants et un bouton Fermer collé en bas. Interface uniquement, aucun changement d'API. Limites : le score de sécurité est calculé dans le navigateur (CVE ouvertes 50 %, en-têtes 30 %, certificats 20 %) ; réactiver une limite chiffrée de Sentinel jamais réglée applique une valeur par défaut (débit 10 req/s, erreurs 4xx 20, limite globale 1000 req/s) ; les textes de la fenêtre ne sont traduits qu'en français et en anglais (espagnol et allemand : anglais). (Admin `0.38.0`)
